...
public class Saturation implements ISpellComponent, IRitualInteraction{
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		if (target instanceof EntityLivingBase){
			...
			if (!world.isRemote)
				((EntityLivingBase)target).addPotionEffect(new PotionEffect(Potion.field_76443_y.id, duration, SpellUtils.instance.countModifiers(SpellModifiers.BUFF_POWER, stack, 0))); // saturation
			return true;
		}
		return false;
	}
	...
	public EnumSet<Affinity> getAffinity(){
		return EnumSet.of(Affinity.NATURE, Affinity.LIFE);
	}
