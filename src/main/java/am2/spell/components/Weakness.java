...
public class Weakness implements ISpellComponent, IRitualInteraction{
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		if (target instanceof EntityLivingBase){
			...
			if (!world.isRemote)
				((EntityLivingBase)target).addPotionEffect(new PotionEffect(Potion.weakness.id, duration, SpellUtils.instance.countModifiers(SpellModifiers.BUFF_POWER, stack, 0))); // saturation
			return true;
		}
		return false;
	}
	...
	public EnumSet<Affinity> getAffinity(){
		return EnumSet.of(Affinity.ICE, Affinity.WATER);
	}
	...
	public Object[] getRecipeItems(){
		return new Object[]{
				new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_GRAY),
				Items.wooden_sword,
				Items.snowball,
				Items.slime_ball,
				Items.brick
